...
const PendingProposal = () => {
    ...
    const RejectProposal = async (proposalId, option) => {
        setRejecting((prevState) => ({
            ...prevState,
            [proposalId]: true,
        }));
        try {
            ...
            swal({
                title: "Proposal " + option + " Successfully",
                icon: 'success'
            });
        } catch (error) {
    ...
    const fetchPendingProposalData = async () => {
        setLoading(true);
        try {
            const result = await backend.call("getProposalAllPending");
            console.log(result);
                ...
                <div className="p-4 border-2 border-gray-200 border-dashed rounded-lg dark:border-gray-700 mt-14">
                    <div className="p-4 bg-white rounded-lg md:p-8 dark:bg-gray-800" id="stats" role="tabpanel" aria-labelledby="stats-tab">
                        <div className="flex flex-row mb-3  justify-between items-center">
                            <h1 className="text-3xl mb-4 font-bold text-left dark:text-white">
                                Pending Proposals
                            </h1>
                                    ...
                                    {proposals.map(proposal => (
                                        <tr key={proposal.id} className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                            ...
                                            <td className="px-2 py-4 flex">
                                                ...
                                                <button
                                                    onClick={() => RejectProposal(proposal.id, "rejected")}
                                                    type="button"
                        ...
                        ) : <p className="text-black dark:text-white text-lg">No pending proposals to display.</p>}
                    </div>
